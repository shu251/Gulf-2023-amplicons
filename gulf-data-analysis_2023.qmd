...
```{r}
unique(supergroup_relabun$Supergroup_simplified)
```
...
(length(unique(tmp_n$FeatureID)) / length(unique(asv_long_avg_wtax_clean$FeatureID)))
# 82% of the data.
head(table_wstn_wdepth)
```
...
asv_long_avg_wtax_clean %>%
  ...
  guides(fill = guide_legend(reverse = TRUE, ncol = 1)) +
  labs(x = "Sequence Relative Abundace", y = "Depth (m)")
# ggsave("figures/nasem_barplot.svg", width = 5, height = 6, device = "svg", limitsize = FALSE)
```
```{r}
...
```{r, fig.width=5, fig.height=6}
tax_nasem <- c("Syndiniales","TSAR-Dinoflagellata", "TSAR-Ciliophora", "TSAR-Gyrista", "TSAR-Radiolaria", "Haptista")
...
asv_long_avg_wtax_clean %>%
  ...
  guides(fill = guide_legend(reverse = TRUE, ncol = 1)) +
  labs(x = "Sequence Relative Abundace", y = "Depth (m)")
# ggsave("figures/nasem_barplot_parasites.svg", width = 5, height = 6, device = "svg", limitsize = FALSE)
```
...
bykmeans_cluster + bykmeans_depth + bykmeans_dist + bykmeans_tax +
  plot_layout(nrow = 1)
# ggsave("figures/bykeamsn_dendrogram-compiled.svg", width = 12, height = 12, device = "svg", limitsize = FALSE)
```
