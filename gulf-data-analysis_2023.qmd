...
* k-means clustering of SAMPLES - help me k-means cluster depths by similar depths and stations. 
* Shannon diversity by T-S (see Linacre et al. 2025), by distance to shore?
* Dino functional ASV trait assignment has too many matches that shouldn't be. Fix this. 
* RDA at Taxon level? and HPRC again? Indicator spp. too.
...
SHANNON <- data.frame(shannon_all) %>% 
  ...
  ggplot(aes(y = depth_order, x = shannon_all, 
             fill = depth_order, shape = FEATURE_ORDER)) +
  geom_jitter(stroke = 0.6, size = 2.5, width = 0.8) +
  scale_shape_manual(values = watercol_shape) +
    scale_fill_manual(values = blues) +
  theme_classic() + theme(
...
PROK <- dendro_annotation %>% 
  ggplot(aes(y = label_order, x = 1, fill = Prok_count)) +
    geom_point(shape = 22, color = "black", size = 4) +
  scale_fill_fermenter(na.value = "grey30", palette = "Reds", direction = 1) +
  scale_y_discrete(position = "right") +
  theme_void() + theme(
    legend.position = "right",
    legend.title = element_blank(),
...
"#d7b6d7","#460e57", 
"#572f0e","#916643","#c95e06", "#dba272", "#b09b10","#b09b10",
"#ad3d46", #Haptista
"#5e0810", "grey30", "#db9399","#93c47d","#ca7ade")

  # "#db1f7d", "#2c5c1d", "#76aee3","#806f8c",
  #               "#f5bfb0", "#93c47d","#7c52bf", "#007a7a", 
  #               "#34495e", "#9b59b6", "#fea02f","#ee429b",
  #               "#f7d472","#647687","#ff6f61","#fcf517","#8a0622",
  #               "#ff7530", "#2986cc", "#e71414", 
  #               "#1abc9c", "#a0b59f", "#d7b6d7", "#23b521", "#5b545c", "black", "white", "grey10", "grey60")


tax_simplified_list <- as.character(rev(sort(unique(asv_long_avg_wtax$Supergroup_simplified))))
tax_simplified_list
...
TAX_TOP_FOUR <- asv_long_avg_wtax %>%
  filter(Supergroup_simplified != "Eukaryota") %>% 
  filter(Supergroup_simplified != "TSAR-NA") %>% 
  filter(Supergroup_simplified != "Obazoa-Metazoa") %>% 
...
gom_dendro_plot + DEPTH + PROFILES + 
  ...
  PROK +  TAX + ASV_TOTAL + (TAX_TOP_FOUR + theme(legend.position = "none")) +
  plot_layout(nrow = 1, 
              widths = c(2.5, 1, 1,
                         1, 1, 1,
                         1, 1, 1,
...
```{r}
# head(asv_long_avg_wtax)
top_taxa_category <- asv_long_avg_wtax %>% 
  filter(!(grepl("Obazoa", Supergroup_simplified))) %>% 
...
save_table1 <- table1 %>% 
  ...
  summarise(Depth_range = paste(min(round(Depth, digits = 1)), max(round(Depth, digits = 1)), sep = "-"),
           Surface_temp = max(round(Temperature, digits = 1)),
           Sample_n = n())
...
rda_summary_table <- gt(df_rda_results %>% 
     select(-FORMULA, -Residual, `Input data` = TEST,
            ...
            Transect, `Number of oil rigs` = OilRig_Count,
            `DCM depth (m)` = DCM, `Salinity maximum depth (m)` = SALT_MAX,
            `Oxygen minimum depth (m)` = O2_MIN, `Day vs. night` = DAY_NIGHT,
            `Hour in day` = HR_OF_DAY, `Maximum depth at station (m)` = DEPTH_MAX,
            `Distance from outflow` = DIST_OUTFLOW, `Depth (m)` = Depth,
            NH4, NO2, NO3, Oxygen, PO4, SIL, Salinity, Temperature)) %>% 
...
```{r}
list_top
# hist(asv_long_avg_wtax$Salinity)
```
...
# | eval: false
# | echo: true
# unique(indic_input$Supergroup_simplified)
indic_input <- asv_long_avg_wtax %>%
  ...
  select(STN_NISKIN, Station, DEPTH_BIN, NH4, NO2, NO3, Oxygen, PO4, SIL, Salinity, SAL_BIN, TEMP_BIN, Temperature, DCM, NO3_BIN, OIL_BIN, OilRig_Count, SALT_MAX, O2_MIN, MEAN_REPS_seq, FeatureID) %>%
  distinct() %>%
...
```{r}
tax_key_annot <- asv_long_avg_wtax %>% 
  select(FeatureID, Taxon, Supergroup_simplified, SUPERGROUP_CLASS, Supergroup, Division, Subdivision, Class, Order, Family, Genus, Species) %>% 
  distinct()
```

### All indicator spp results
```{r}
results_indic_summary <- rbind(dcm_bin, o2_bin, depth_indic_bin, no3_bin, temp_bin, sal_bin,stn_bin) %>% 
...
results_indic_full <- rbind(dcm_bin, o2_bin, depth_indic_bin, no3_bin, temp_bin, sal_bin, stn_bin) %>% 
  left_join(tax_key_annot) %>%  
...
results_indic_full %>% ungroup() %>% 
  filter(!is.na(Supergroup_simplified)) %>% 
  select(Supergroup_simplified, FeatureID, starts_with("Indicator_TEMP"), starts_with("Indicator_NO3")) %>% 
  pivot_longer(cols = starts_with("Indicator_"), names_to = "indicator_list", values_to = "asv_count") %>% 
  filter(!is.na(asv_count)) %>% distinct() %>% 
  group_by(Supergroup_simplified, FeatureID, indicator_list) %>% 
    summarise(SUM = sum(asv_count)) %>% 
...
```{r}
head(tmp)
tmp2 <- (tmp %>% select(FeatureID, Order) %>% distinct())
table(tmp2$Order)

tmp3 <- asv_long_avg_wtax %>%
  filter(Class != "Syndiniales") %>% 
  filter(Supergroup_simplified == "TSAR-Dinoflagellata") %>% 
  select(FeatureID, Order) %>% distinct()
table(tmp3$Order)
```
...
dino_trait_asv <- asv_long_avg_wtax %>% 
  ...
  left_join(tax_compiled_df) %>% # Added TAXA to look at taxa of interest.
  mutate(MORPHOTYPE = case_when(
    ...
    grepl("CV", morphotype) ~ "Myzocytosis",
...
# unique(dino_trait_asv$TAXA)
```


```{r}
supp_dino_fxn <- dino_trait_asv %>% 
  ungroup() %>% 
  select(FeatureID, TAXA, Supergroup_simplified, Subdivision, Class, Order, feeding_mode, morphotype, MORPHOTYPE) %>%
  # group_by(Supergroup_simplified, TAXA, feeding_mode, morphotype) %>% 
  group_by(Supergroup_simplified, TAXA, Subdivision, Class, feeding_mode, morphotype, MORPHOTYPE) %>%
  summarise(ASV_COUNT = n()) 
write.csv(supp_dino_fxn, file = "output-tables/fxntrait-dino-summary.csv")
```
...
tmp <- dino_trait_asv %>% 
  filter(Class == "Syndiniales") %>%
  select(FeatureID, Order, MORPHOTYPE) %>% distinct()
  # filter(Order == "Dino-Group-IV")
  # filter(morphotype == "CM")
table(tmp %>% select(Order, MORPHOTYPE))
unique(tmp$feeding_mode)
# unique(tmp$MORPHOTYPE)
View(tmp)
# View(dino_trait_asv %>% 
...
depth_bin <- c("< 5m","5-25m","25-60m",
                          "60-270m","270-400m","400-800m",
                          "800-1200m","1200-2000m","2000-3000m")
# table(dino_trait_asv$feeding_mode)
# View(dino_trait_asv)
# dino_trait_asv
```
...
dino_trait_asv %>% 
  ungroup() %>% 
  select(FeatureID, STN_NISKIN, DEPTH_BIN, Order, Class, feeding_mode, morphotype, MORPHOTYPE) %>% 
  ...
  filter(morphotype != "Parasite") %>% 
  group_by(DEPTH_BIN, MORPHOTYPE) %>% 
  summarise(ASV_COUNT = n()) %>% 
  mutate(DEPTH_BIN_ORDER = factor(DEPTH_BIN, levels = rev(depth_bin))) %>% 
  ggplot(aes(x = ASV_COUNT, y = DEPTH_BIN_ORDER, fill = MORPHOTYPE)) +
  geom_bar(stat = "identity", position = "stack", color = "black", width = 0.6) +
    scale_fill_manual(values = c("#bf4e40", "#056f00", "#36305a","#b9dcf4", "#dec3e0")) +
    scale_x_continuous(expand = c(0,0)) +
  theme_classic() +
    theme(axis.text.x = element_text(color = "black", size = 10, face = "bold"),
          axis.text.y = element_text(color = "black", size = 10, face = "bold"),
...
# FROM TAXA - test these with PLS
Dinophyceae;Gymnodiniales
Dinophyceae;Gonyaulacales

Syndiniales;Dino-Group-I
Syndiniales;Dino-Group-II
Syndiniales;Dino-Group-III
Syndiniales;Dino-Group-IV
Syndiniales;Dino-Group-V
...
Spirotrichea;Oligotrichida
Oligohymenophorea;Scuticociliatia_1
Phyllopharyngea;Cyrtophoria

Coscinodiscophyceae;Rhizosoleniales
Mediophyceae;Chaetocerotales
Mediophyceae;Thalassiosirales
...
Questions to ask with this data.

If we separate "Dinophyceae;Gymnodiniales" and "Dinophyceae;Gonyaulacales" as strict heterotrophs versus, able to incorporate some kind of phototrophy - do we get a better predictive set of parameters?

How about parasitic vs. not completely parasitic?



Center & scale like in the RDA analysis.

Metadata (Same code as used in RDA)
```{r}
categorical <- table1_wfeature %>% 
  select(STN_NISKIN, 
  DCM, O2_MIN) %>% distinct()
...
```{r}
# pls(predictors, responses)
model_dinos <- pls(standarize_metadata, clr_asv_matrix,
                   ncomp = 8, center = TRUE, scale = TRUE,
                 cv = 1, method = "simpls",
                 lim.type = "ddmoments", ncomp.selcrit = "min")
# summary(model_dinos)
```

```{r}
plot(model_dinos)
plotRMSE(model_dinos)
plotYVariance(model_dinos)
plotXYResiduals(model_dinos)
plotXYScores(model_dinos)
# outlier_dino <- setDistanceLimits(pls_dinos, lim.type = "ddrobust")
# plotXYResiduals(outlier_dino)
```

```{r}
print(model_dinos$res)
```
