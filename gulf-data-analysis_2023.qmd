...
library(plotly); library(viridis); library(vegan); library(treemapify)
```
...
> Above plot shows hypothetical numbers of k clusters (x-axis) with the variance within the cluster (y-axis). We want to isolate the number of clusters where we cover as much variance as possible - BUT if we add more clusters, not much will be helpful. This means "the bend" here. In this case, We are going to go with 5 clusters.

Use chosen number of clusters (n=5) to compute kmeans.

```{r}
# Perform kmeans with 5 centers
kmeans_seq_5 <- kmeans(standarize_seq, centers = 5)
...
# save(kmeans_membership_5, file = "input-data/kmeans-membership-seqs-5.RData")
```
...
Join metadata together with the cluster information
```{r}
...
table_supp1 <- table_wfeature %>%
  ...
  select(Station, Niskin, STN_NISKIN, Depth, DEPTH_BIN, Latitude, Longitude, 
         Date, Time_UTC, DAY_NIGHT, TRANSECT, DIST_OUTFLOW, CLUSTER,
         OilRig_Count, MLD, SAL_MAX, O2_MIN, DCM, Temperature, Salinity, 
         DIC, NH4, NO2, NO3, PO4, SIL, Oxygen_CTD, TA, pH, Prok_count, Sequence_files, Total_ASVs, Total_sequences)
...
CSWr - Caribbean Surface Water Remnant
GCW - Gulf Common Water
NADW - North Atlantic Deep Water
NASUW - North Atlantic Subtropical Underwater
TACW - Tropical Atlantic Central Water
TACWn - Tropical Atlantic Central Water (nucleus)

## Plot K-means T-S plot
...
(SHANNON_) / ((PCA_PLOT_depth) + theme(legend.position = "none") + (ts_cluster + theme(legend.position = "right"))) + plot_layout(nrow = 2, heights = c(0.8, 1, 1)) + plot_annotation(tag_levels = c("A"))
# ggsave("figures/diversity-pca-ts-plot.svg", width = 8, height = 7, device = "svg", limitsize = FALSE)
```
```{r, fig.width=12, fig.height=12}
((ts_cluster + theme(legend.position = "left")) + (PCA_PLOT_kmeans) + theme(legend.position = "none")) / (SHANNON_) + plot_layout(nrow = 2, heights = c(1, 1, 0.8)) + plot_annotation(tag_levels = c("A"))
# ggsave("figures/ts-pca-diversity-plot_2.svg", width = 12, height = 12, device = "svg", limitsize = FALSE)
```
...
scale_fill <- list(
  ...
  scale_fill_manual(values = stn_order_color, guide = guide_legend(reverse = FALSE, order = 3)), # station
  scale_fill_manual(values = c("#d8d8f0", "#181752"), guide = guide_legend(order = 4)), #MLD
  ...
  scale_fill_viridis_c(option = "magma", direction = -1, guide = guide_legend(reverse = FALSE, order = 5)), #dist
  scale_fill_manual(values = c("#efaaf2", "#f003fc", "#470d4a"), guide = guide_legend(reverse = FALSE, order = 6)), #transect
  scale_fill_manual(values = c("grey90","#e38aa6","#ba4e70","#a13356","#8a1a3d","#54051e"), guide = guide_legend(reverse = FALSE, order = 7)), #Oil rig count
  scale_fill_manual(values = c("#96bbe0","#0a3157"), guide = guide_legend(reverse = TRUE, order = 8)), #DAy night
  scale_fill_fermenter(na.value = "grey50", palette = "Reds", direction = 1, guide = guide_legend(order = 9)), # proks
  ## Nutrients
  scale_fill_viridis(option = "plasma", guide = guide_legend(order = 10)),
...
```
### Dashboard data: sequence abundance
...
supergroup_relabun <- asv_long_avg_wtax_clean %>%
  ...
  filter(Supergroup_simplified != "TSAR-NA") %>% 
  filter(Supergroup_simplified != "Obazoa-Metazoa") 
# write.csv(supergroup_relabun, file = "dashboard-data/supergroup_relabun.csv")
```
...
* Add another filter on this... for ASVs have to appear in more than 25% of the samples. Total sequence counts for the taxa need to also be pretty high...
...
```{r}
dinos_coastal <- dinos_curated %>% 
  filter(Station %in% coastal_stn) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
  separate(TAXA, into = c("GROUP", "SUBGROUP"), sep = ";", remove = FALSE) %>% 
  mutate(SUBGROUP = str_replace_na(SUBGROUP, "Other")) %>% 
  add_column(CATEGORY = "Coastal")

dinos_offshore <- dinos_curated %>% 
  filter(!(Station %in% coastal_stn)) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
  separate(TAXA, into = c("GROUP", "SUBGROUP"), sep = ";", remove = FALSE) %>% 
  mutate(SUBGROUP = str_replace_na(SUBGROUP, "Other")) %>% 
  add_column(CATEGORY = "Offshore")

dinos_shallow <- dinos_curated %>% 
  filter(Depth < 200) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
  separate(TAXA, into = c("GROUP", "SUBGROUP"), sep = ";", remove = FALSE) %>% 
  mutate(SUBGROUP = str_replace_na(SUBGROUP, "Other")) %>% 
  add_column(CATEGORY = "Shallow")


dinos_deep <- dinos_curated %>% 
  filter(Depth >= 200) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
  separate(TAXA, into = c("GROUP", "SUBGROUP"), sep = ";", remove = FALSE) %>% 
  mutate(SUBGROUP = str_replace_na(SUBGROUP, "Other")) %>% 
  add_column(CATEGORY = "Deep")

dino_tree <- rbind(dinos_coastal, dinos_offshore, dinos_shallow, dinos_deep) %>% 
  mutate(CATEGORY_ORDER = factor(CATEGORY, levels = c("Coastal", "Shallow", "Offshore", "Deep")))
tax_dino_order <- (sort(unique(dino_tree$SUBGROUP)))
...
```{r}
dino_tree_plot <- ggplot(dino_tree, aes(area = SUM, fill = SUBGROUP, 
                      label = GROUP,
                      subgroup = GROUP, 
                      subgroup2 = TAXA)) + 
  geom_treemap() +
  scale_fill_manual(values = dino_colors) +
  facet_wrap(vars(CATEGORY_ORDER), nrow = 2) +
  geom_treemap_subgroup_border(colour = "white", size = 3) +
  geom_treemap_subgroup2_border(colour = "white", size = 1) +
  geom_treemap_subgroup_text(colour = "white", alpha = 0.8, fontface = "bold") +
  theme_minimal() +
...
rad_tree <- rbind(coastal, offshore, shallow, deep) %>% 
mutate(CATEGORY_ORDER = factor(CATEGORY, levels = c("Coastal", "Shallow", "Offshore", "Deep")))
tax_rad_order <- (sort(unique(rad_tree$SUBGROUP)))
tax_rad_order
...
```{r}
rad_tree_plot <- ggplot(rad_tree, aes(area = SUM, fill = SUBGROUP, 
                      label = GROUP,
                      subgroup = GROUP, 
                      subgroup2 = TAXA)) + 
  geom_treemap() +
...
```{r}
# head(gyrista_curated)
# unique(gyrista_curated$TAXA)
```

```{r}
coastal <- gyrista_curated %>% 
  filter(Station %in% coastal_stn) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
...
gyrista_tree <- rbind(coastal, offshore, shallow, deep) %>% 
  mutate(CATEGORY_ORDER = factor(CATEGORY, levels = c("Coastal", "Shallow", "Offshore", "Deep")))
tax_gyrista_order <- (sort(unique(gyrista_tree$SUBGROUP)))
tax_gyrista_order
...
unique(gyrista_tree$GROUP)
```

```{r}
gyrista_tree_plot <- ggplot(gyrista_tree, aes(area = SUM, fill = SUBGROUP, 
                      # label = GROUP,
                      subgroup = GROUP, 
                      subgroup2 = TAXA)) + 
  geom_treemap() +
...
```{r}
head(ciliate_curated)
unique(ciliate_curated$TAXA)
```

```{r}
coastal <- ciliate_curated %>% 
  filter(Station %in% coastal_stn) %>% 
  group_by(TAXA) %>% 
    summarise(SUM = sum(MEAN_REPS_seq),
              COUNT = n()) %>% 
...
ciliate_tree <- rbind(coastal, offshore, shallow, deep) %>% 
mutate(CATEGORY_ORDER = factor(CATEGORY, levels = c("Coastal", "Shallow", "Offshore", "Deep")))
tax_ciliate_order <- (sort(unique(ciliate_tree$SUBGROUP)))
...
```{r}
ciliate_tree_plot <- ggplot(ciliate_tree, aes(area = SUM, fill = SUBGROUP, 
                      label = GROUP,
                      subgroup = GROUP, 
                      subgroup2 = TAXA)) + 
  geom_treemap() +
...
dino_tree_plot + rad_tree_plot +
  gyrista_tree_plot + ciliate_tree_plot +
  plot_layout(nrow = 2) + plot_annotation(tag_levels = "A")
ggsave("figures/tree-fourpanel.svg", width = 12, height = 10, device = "svg", limitsize = FALSE)
```
...
toptaxa_surf_clr <- function(DF, TITLE){
  ...
  (data.frame(clr_surf) %>% 
    ...
    geom_jitter(color = "white", fill = "black", size = 2) +
    scale_shape_manual(values = c(21, 22, 23)) +
    facet_grid(rows = vars(TAXA), scales = "free_x") +
    theme_classic() +
    theme(axis.text.x = element_text(color = "black", angle = 0, size = 8,
          ...
          legend.position = "left",
          aspect.ratio = 1/2) +
    labs(x = "km", y = "Center log-ratio", title = TITLE)) 
}
```
