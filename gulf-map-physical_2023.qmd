...
```
### Four main taxa- surface

```{r}
# select_four <- c("TSAR-Dinoflagellata", "TSAR-Radiolaria","TSAR-Gyrista", "TSAR-Ciliophora")
head(bytaxa_surface_asvs_FOUR)
```
...
# ggsave("figures/surface-main-four.svg", width = 14, height = 10, device = "svg", limitsize = FALSE)
```
...
```{r}
unique(asv_long_avg_wtax_clean$SUPERGROUP_CLASS)
```

```{r, fig.height=5, fig.width=17, warning=FALSE}
tax_whole_asv_profile("Haptophyta_X-Prymnesiophyceae", 0, 250, "Haptophyta_X-Prymnesiophyceae")
# ggsave("figures/gyrista-dictyocho.svg", width = 15, height = 5, device = "svg", limitsize = FALSE)
```
```{r}
view(asv_long_avg_wtax_clean)
```
...
ggplot(data = world) +
  ...
  theme(legend.position = "none",
    ...
    plot.background = element_blank(),
        panel.border = element_rect(fill = NA))
# ggsave("figures/kmeans-surface-stations.svg", width = 7, height = 7, device = "svg", limitsize = FALSE)
```
## Depth k-means
...
# ggsave("figures/kmeans-stations-bydepth.svg", width = 9, height = 4, device = "svg", limitsize = FALSE)
```
