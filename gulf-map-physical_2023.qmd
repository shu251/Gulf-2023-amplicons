...
```{r}
load(file = "input-data/gulf-2023-sequence-analysis_12222025.RData", verbose = TRUE)
...
> T-S plot with all stations, and depths above the DCM? above the O2 min layer?

```{r}
# master_metadata %>% 
#   select(Station, Niskin, DEPTH = `CTD Recorded Depth (m)`, TEMP = `CTD Temperature`, SALINITY = `Corrected CTD Salinity`, PRESSURE = `CTD Pressure (db)`,
#          LONG = `NMEA Longitude`, LAT = `NMEA Latitude`)
```
...
Measure of stability within a sampled vertical profile. Use the formula `N = √(g/`ρ`) (∂ρ/∂z)`
`g` = gravity 9.8 m/s^2
`ρ` = density
`z` = depth

> Calculate MLD, would add this to my parameters for DCM and more.
...
bv_output <- by_freq %>% 
  ...
  reframe(bv_out = gsw_Nsquared(by_freq$SALINITY, 
                                    by_freq$TEMP, 
                                    by_freq$PRESSURE, 
                                    by_freq$LAT)) %>% 
  mutate(BV_INDIC = names(bv_out)) %>% 
  unnest(bv_out) %>% 
  pivot_wider(names_from = BV_INDIC, values_from = bv_out)

# head(tmp)
# unique(tmp$BV_INDIC)
# ?gsw_Nsquared()
  # bind_cols(data.frame(gsw_Nsquared(by_freq$SALINITY, 
  #                                   by_freq$TEMP, 
  #                                   by_freq$PRESSURE, 
  #                                   by_freq$LAT)))
```

Interpret B-V frequency to determine Mixed Layer Depth.
```{r}

```


# Plot water column parameters
...
```{r}
head(env_metadata_mod)
unique(env_metadata_mod$FEATURE)
```
...
env_metadata_mod %>% 
  ...
  filter(FEATURE_ORDER != "Depth with no feature") %>% 
  ggplot(aes(x = STN_ORDER, y = Depth, 
             shape = FEATURE_ORDER, fill = FEATURE_ORDER, group = FEATURE_ORDER)) +
  geom_line(aes(color = FEATURE_ORDER)) + 
  geom_point(color = "black", stat = "identity",
             size = 3) +
  facet_grid(cols = vars(Transect), scale = "free") +
  ...
  labs(x = "Station (coast to offshore)", y = "Depth (m)")
  
```


> Goal: what water masses are influencing key stations where I see diversity-based edge effects? specific taxa? 


# Calculate distance from MS River plume

> This would replace additional parameters, would test for other key factors.
...
surf_all_asv <- interpolate_lat_long_asv(surface_asv_odv, "ASV_COUNT", 500, 3000, "Total ASVs")
surf_all_asv
```
...
interpolate_lat_long_asv(surface_asv_odv, "ASV_COUNT", 400, 2000, "Total ASVs") +
interpolate_depth_long_asv(depth_asv_odv, "transect1","ASV_COUNT", 400, 2000,"Total ASVs") +
  interpolate_depth_long_asv(depth_asv_odv, "transect2","ASV_COUNT", 400, 2000, "Total ASVs") +
  interpolate_depth_long_asv(depth_asv_odv, "transect3","ASV_COUNT", 400, 2000, "Total ASVs") +
...
interpolate_depth_long_asv((depth_asv_odv %>% filter(Depth <= 200)), "transect1","ASV_COUNT", 400, 2000,"Total ASVs") +
  interpolate_depth_long_asv((depth_asv_odv %>% filter(Depth <= 200)), "transect2","ASV_COUNT", 400, 2000, "Total ASVs") +
  interpolate_depth_long_asv((depth_asv_odv %>% filter(Depth <= 200)), "transect3","ASV_COUNT", 400, 2000, "Total ASVs") +
  plot_layout(nrow = 1, guides = "collect") + plot_annotation(tag_levels = c("a"))
# ggsave("figures/asv-map-all.svg", width = 15, height = 5, device = "svg", limitsize = FALSE)
