...
```{r}
# | eval: false
# save(samples_too_low, asv_long_cleaned, file =  "input-data/asv_wtax_qc_GoM23_102025.RData")
```


# Curate ASV tables and include metadata
...
#   # First average ASV sequence count across replicates
  group_by(FeatureID, Taxon, 
           ...
           STN_NISKIN, Station, Niskin, Depth, Latitude, Longitude, 
           Pressure, Temperature, Salinity, Oxygen, NH4, NO2, NO3, PO4, SIL, Prok_count,
...
```{r}
asv_long_avg_wtax <- asv_long_avg_wtax %>% 
  filter(FeatureID %in% asvs_to_keep)

asv_long_cleaned_wenv_mod <- asv_long_cleaned_wenv_mod %>% 
  filter(FeatureID %in% asvs_to_keep)
```
...
```{r}
# | eval: false
# save(asv_long_cleaned_wenv_mod,
#      asv_long_avg_wtax, tax_key,
#      env_metadata_mod,
#      file = "input-data/gulf-2023-sequence-analysis_12222025.RData")
```
