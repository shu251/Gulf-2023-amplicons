...
### List of ASVs with too low sequences


Starting with 59,594 total ASVs.
```{r}
tmp <- asv_long_cleaned_wenv_mod %>% 
  group_by(FeatureID) %>% 
  summarize(SUM = sum(SEQUENCE_COUNT)) %>% 
  filter(SUM >= 100)
# str(tmp)
# hist(tmp$SUM)

asvs_less_100 <- as.character(unique(tmp$FeatureID))
```
...
save(asv_long_cleaned_wenv, asv_long_cleaned_wenv_mod,
     ...
     env_metadata_mod, asvs_less_100, 
